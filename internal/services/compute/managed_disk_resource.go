...
func resourceManagedDiskCreate(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	platformKeyEncryptionType := disks.EncryptionTypeEncryptionAtRestWithPlatformKey
	...
	props := &disks.DiskProperties{
		...
		Encryption: &disks.Encryption{
			Type: &platformKeyEncryptionType,
		},
	}
	...
	if d.Get("trusted_launch_enabled").(bool) {
		trustedLaunchDiskSecurityType := disks.DiskSecurityTypesTrustedLaunch
		props.SecurityProfile = &disks.DiskSecurityProfile{
			SecurityType: &trustedLaunchDiskSecurityType,
		}
	...
	if v, ok := d.GetOk("hyper_v_generation"); ok {
		diskHyperVGeneration := disks.HyperVGeneration(v.(string))
		props.HyperVGeneration = &diskHyperVGeneration
	}
