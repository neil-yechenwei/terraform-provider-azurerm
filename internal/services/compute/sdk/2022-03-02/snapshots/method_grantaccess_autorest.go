...
func (c SnapshotsClient) GrantAccess(ctx context.Context, id SnapshotId, input GrantAccessData) (result GrantAccessOperationResponse, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "snapshots.SnapshotsClient", "GrantAccess", nil, "Failure preparing request")
		return
	}

	result, err = c.senderForGrantAccess(ctx, req)
	if err != nil {
		err = autorest.NewErrorWithError(err, "snapshots.SnapshotsClient", "GrantAccess", result.HttpResponse, "Failure sending request")
		return
	}
...
func (c SnapshotsClient) preparerForGrantAccess(ctx context.Context, id SnapshotId, input GrantAccessData) (*http.Request, error) {
...
func (c SnapshotsClient) senderForGrantAccess(ctx context.Context, req *http.Request) (future GrantAccessOperationResponse, err error) {
