...
func resourceCosmosDbAccountUpdate(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	if model := resp.Model; model != nil {
		if props := model.Properties; props != nil {
			account.Properties.EnableMultipleWriteLocations = props.EnableMultipleWriteLocations
		}
	}
...
func flattenCosmosdbAccountBackup(input cosmosdb.BackupPolicy) ([]interface{}, error) {
	...
	if _, ok := input.(cosmosdb.ContinuousModeBackupPolicy); ok {
		return []interface{}{
			map[string]interface{}{
				"type": string(cosmosdb.BackupPolicyTypeContinuous),
			},
		}, nil
	} else if v, ok := input.(cosmosdb.PeriodicModeBackupPolicy); ok {
		var interval, retention int
		if v := v.PeriodicModeProperties.BackupIntervalInMinutes; v != nil {
			interval = int(*v)
		}
		if v := v.PeriodicModeProperties.BackupRetentionIntervalInHours; v != nil {
			retention = int(*v)
		}
		var storageRedundancy cosmosdb.BackupStorageRedundancy
		if v.PeriodicModeProperties.BackupStorageRedundancy != nil {
			storageRedundancy = pointer.From(v.PeriodicModeProperties.BackupStorageRedundancy)
		}
		return []interface{}{
			map[string]interface{}{
