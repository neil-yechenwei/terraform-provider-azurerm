...
layout: "azurerm"
page_title: "Azure Resource Manager: azurerm_workloads_sap_central_server_instance"
description: |-
  Manages a SAP Central Server Instance.
---

# azurerm_workloads_sap_central_server_instance
...
resource "azurerm_workloads_sap_central_server_instance" "example" {
  name                    = "example-sapcentral"
  resource_group_name     = azurerm_resource_group.example.name
  location                = azurerm_resource_group.example.location
...
* `name` - (Required) The name which should be used for this SAP Central Server Instance. Changing this forces a new resource to be created.

* `resource_group_name` - (Required) The name of the Resource Group where the SAP Central Server Instance should exist. Changing this forces a new resource to be created.

* `location` - (Required) The Azure Region where the SAP Central Server Instance should exist. Changing this forces a new resource to be created.
...
* `tags` - (Optional) A mapping of tags which should be assigned to the SAP Central Server Instance.

## Attributes Reference

In addition to the Arguments listed above - the following Attributes are exported:

* `id` - The ID of the SAP Central Server Instance.

## Timeouts

The `timeouts` block allows you to specify [timeouts](https://www.terraform.io/docs/configuration/resources.html#timeouts) for certain actions:

* `create` - (Defaults to 60 minutes) Used when creating the SAP Central Server Instance.
* `read` - (Defaults to 5 minutes) Used when retrieving the SAP Central Server Instance.
* `update` - (Defaults to 60 minutes) Used when updating the SAP Central Server Instance.
* `delete` - (Defaults to 60 minutes) Used when deleting the SAP Central Server Instance.

## Import

SAP Central Server Instances can be imported using the `resource id`, e.g.

```shell
terraform import azurerm_workloads_sap_central_server_instance.example /subscriptions/00000000-0000-0000-0000-000000000000/resourceGroups/resourceGroup1/providers/Microsoft.Workloads/sapVirtualInstances/sapVirtualInstance1/centralInstances/centralInstance1
```
